...
uint16_T aux_gTimCnt_APT =0, TimFactor_APT = 10; //auxiliar timer variable updated by irq used to obtain ms
...
//************************************************************************
void CANOpenHW_SendMessage_APT(CAN_MSG_APT *pTransmitBuf_APT, uint32_T *txID_APT, uint8_T *txDLC_APT, uint8_T *txData_APT)
{
	memcpy(txData_APT, pTransmitBuf_APT->BUF_APT, pTransmitBuf_APT->LEN);
	*txID_APT = pTransmitBuf_APT->ID;
	*txDLC_APT = pTransmitBuf_APT->LEN;
}


//*************************************************************************
uint8_T CANOpenHW_IsTimeExpired_APT(uint16_T timestamp_APT)
{
    if (gTimCnt_APT>timestamp_APT)
    {
        if((gTimCnt_APT-timestamp_APT)<0x8000)
            return 1;
         else
          return 0;
    }
    ...
    {
       if((timestamp_APT-gTimCnt_APT)>0x8000)
         return 1;
      else
        return 0;
   }
    ...
    {// prepare all TPDOs for transmission
        ...
        {// This TPDO is used
         // Copy current process data
            memcpy(&gTPDOConfig_APT[i].CAN.BUF_APT[0],gTPDOConfig_APT[i].pData,gTPDOConfig_APT[i].CAN.LEN);
            #ifdef USE_EVENT_TIME_APT
            //Reset event timer for immediate transmission
            gTPDOConfig_APT[i].event_timestamp_APT=CANOpenHW_GetTime_APT();
            #endif
            #ifdef USE_INHIBIT_TIME_APT
            gTPDOConfig_APT[i].inhibit_status_APT=2;// Mark as ready for transmission
            //Reset event timer for immediate transmission
            gTPDOConfig_APT[i].inhibit_timestamp_APT=CANOpenHW_GetTime_APT();
            #endif
        }
        i++;
    }
...
//*************************************************************************
void TransmitPDO_APT(BYTE PDONr, uint32_T *txID_APT, uint8_T *txDLC_APT, uint8_T *txData_APT)
{
#ifdef USE_INHIBIT_TIME_APT
    gTPDOConfig_APT[PDONr].inhibit_status_APT=1;// New inhibited timer started
    gTPDOConfig_APT[PDONr].inhibit_timestamp_APT=CANOpenHW_GetTime_APT()+gTPDOConfig_APT[PDONr].inhibit_time_APT;
#endif
#ifdef USE_EVENT_TIME_APT
    gTPDOConfig_APT[gTPDONr_APT].event_timestamp_APT=CANOpenHW_GetTime_APT()+gTPDOConfig_APT[gTPDONr_APT].event_time_APT;
#endif
    CANOpenHW_SendMessage_APT(&gTPDOConfig_APT[PDONr].CAN, txID_APT, txDLC_APT, txData_APT);
}
#endif //NR_OF_TPDOS>0


//*********************  CANOpenSlave_Init ************************************//
void CANOpenSlave_Init_APT(WORD Baudrate_APT, BYTE Node_ID_APT, WORD Heartbeat_APT)
{
   BYTE i;
   #ifdef INTERNAL_RX_QUEUE
   can_rx_queue_APT.in = 0;
   can_rx_queue_APT.out = 0;
   can_rx_queue_APT.ovf_cnt_APT = 0;
   #endif
   gCANOPENConfig_APT.Node_ID_APT=Node_ID_APT;
   gCANOPENConfig_APT.error_code_APT=0;
   gCANOPENConfig_APT.Baudrate_APT=Baudrate_APT;
   gCANOPENConfig_APT.heartbeat_time_APT=Heartbeat_APT;
   gCANOPENConfig_APT.heartbeat_msg_APT.ID=0x700+Node_ID_APT;
   gCANOPENConfig_APT.heartbeat_msg_APT.LEN=1;
   gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0; //Current NMT state of this node = bootup
   gCANOPENConfig_APT.error_register_APT=0;

   //Init SDO Response/Abort message
   gTxSDO_APT.ID=0x580+gCANOPENConfig_APT.Node_ID_APT;
   gTxSDO_APT.LEN=8;
...
#if NR_OF_RPDOS_APT>0
void CANOpenSlave_InitRPDO_APT(BYTE PDO_NR_APT, WORD CAN_ID, BYTE len, BYTE *dat)
{
	PDO_NR_APT--;
	gRPDOConfig_APT[PDO_NR_APT].LEN=len;
	gRPDOConfig_APT[PDO_NR_APT].pData=dat;
	if (CAN_ID==0)
	{
		gRPDOConfig_APT[PDO_NR_APT].ID=0x200+(0x100*((WORD)(PDO_NR_APT)))+gCANOPENConfig_APT.Node_ID_APT;
	}
	else
	{
		gRPDOConfig_APT[PDO_NR_APT].ID=CAN_ID;
	}
}
#endif //NR_OF_RPDOS>0
...
#if NR_OF_TPDOS_APT
void CANOpenSlave_InitTPDO_APT(BYTE PDO_NR_APT, WORD CAN_ID, WORD event_time_APT, WORD inhibit_time_APT, BYTE len, BYTE *pDat)
{
	PDO_NR_APT--;
	gTPDOConfig_APT[PDO_NR_APT].CAN.ID=CAN_ID;
	gTPDOConfig_APT[PDO_NR_APT].CAN.LEN=len;
	gTPDOConfig_APT[PDO_NR_APT].pData=pDat;
	#ifdef USE_EVENT_TIME_APT
	gTPDOConfig_APT[PDO_NR_APT].event_time_APT=event_time_APT;
	#endif
	#ifdef USE_INHIBIT_TIME_APT
	gTPDOConfig_APT[PDO_NR_APT].inhibit_time_APT=inhibit_time_APT;
	#endif
}
#endif // NR_OF_TPDOS>0
...
void CANOpenSlave_ResetCommunication_APT(void)
{
    CANOpenSlave_Init_APT(500, OD_NODEID_APT, OD_HEARTBEAT_APT);
        ...
        CANOpenSlave_InitTPDO_APT(1,   0x00 ,  500,  0, 8,  &TPDO01_APT.Message);  // APT
    #endif
...
BYTE ReplyWith_APT(BYTE *pDat, BYTE len)
{
  ...
  gTxSDO_APT.BUF_APT[0] = 0x43 | ((4-len) << 2);
  // copy "backwards" to swap byte order to lowest first
  for (k = (len - 1); k >= 0; k--)
  {
    gTxSDO_APT.BUF_APT[4+k] = *pDat;
    pDat++;
  }
...
//*************************************************************************
void Send_SDO_Abort_APT(DWORD ErrorCode,uint32_T *txID_APT,uint8_T *txDLC_APT, uint8_T *txData_APT)
{
    BYTE i;
    gTxSDO_APT.BUF_APT[0]=0x80;
    for(i=0;i<4;i++)
    {
        gTxSDO_APT.BUF_APT[4+i]=ErrorCode;
        ErrorCode>>=8;
        CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
    }
}
...
//*************************************************************************
BYTE Handle_SDO_Request_APT(BYTE *pData,uint32_T *txID_APT,uint8_T *txDLC_APT, uint8_T *txData_APT)
{ //Returns 1 if SDO Access success
  //Returns 0 if SDO Access Abort generated
    ...
    gTxSDO_APT.BUF_APT[1]=pData[1];// index lo
    gTxSDO_APT.BUF_APT[2]=pData[2];//index hi
    gTxSDO_APT.BUF_APT[3]=pData[3];// subindex
   ...
   //It is a read or write command
    if ((cmd==0x40)||(cmd==0x20))
    {
     ...
     if (found != 0xFF)
     {
      ...
      if (cmd == 0x40)
      {
        ...
        {
          Send_SDO_Abort_APT(SDO_ABORT_UNSUPPORTED,txID_APT, txDLC_APT, txData_APT);
          return 0;
        }
      }
      // write command?
      else
      {
        ...
        if ((pOD->len & ODWR) != 0)
        {
          ...
          {
            len--;
            gProcImg_APT[pOD->offset+len] = gRxCAN_APT.BUF_APT[4+len];
          }
		  // write response
          gTxSDO_APT.BUF_APT[0] = 0x60;
          CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
 		  return 1;
        }
        // write not allowed
    ...
    if (found < 255)
    {
      ...
      if (cmd == 0x40)
      {
        memcpy(&gTxSDO_APT.BUF_APT[0],&(SDOResponseTable_APT[(found*8)]),8);
        CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
        return 1;
      }
      // write command
      Send_SDO_Abort_APT(SDO_ABORT_READONLY,txID_APT, txDLC_APT, txData_APT);
      return 0;
    }
    if ((index == 0x1001) && (subindex == 0x00))
    {
      ...
      if (cmd == 0x40)
      {
        ...
        gTxSDO_APT.BUF_APT[0] = 0x4F;
        gTxSDO_APT.BUF_APT[4] = gCANOPENConfig_APT.error_register_APT;
        CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
        return 1;
      }
      // write command
      ...
      // Hard coding of dynamic read/write accesses
        ...
        {//Acces to [1017,00] - heartbeat time
            if (cmd==0x40)
            {//Read command
                gTxSDO_APT.BUF_APT[0]=0x4B;// expedited, 2-bytes of data
                gTxSDO_APT.BUF_APT[4]=(BYTE) gCANOPENConfig_APT.heartbeat_time_APT;
                gTxSDO_APT.BUF_APT[5]=(BYTE) (gCANOPENConfig_APT.heartbeat_time_APT>>8);
                CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
                return 1;
            }
            if (*pData==0x2B)
            {// Expedited write command with 2-bytes of data
               gCANOPENConfig_APT.heartbeat_time_APT=pData[5];
               gCANOPENConfig_APT.heartbeat_time_APT=(gCANOPENConfig_APT.heartbeat_time_APT<<8)+pData[4];
               gTxSDO_APT.BUF_APT[0]=0x60;// Write response
               //Needed to pass conformance test: clear unused bytes
               gTxSDO_APT.BUF_APT[4]=0;
               gTxSDO_APT.BUF_APT[5]=0;
               gTxSDO_APT.BUF_APT[6]=0;
               gTxSDO_APT.BUF_APT[7]=0;
               CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
               return 1;
            }
            Send_SDO_Abort_APT(SDO_ABORT_UNSUPPORTED,txID_APT, txDLC_APT, txData_APT);
    ...
    // See if the data requested is in the OD
       ...
       {//OD entry found
           if (cmd==0x40)
           {// Read command
               CopyConstToData_APT(&gTxSDO_APT.BUF_APT[0],&SDOResponseTable_APT[(found*8)]);
               CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
               return 1;
           }
           // Write command
       ...
       if ((index==0x1001)&&(subindex==0x00))
        {
            if(cmd==0x40)
            {// Read command
                gTxSDO_APT.BUF_APT[0]=0x4F;//Expedited, 1-byte of data
                gTxSDO_APT.BUF_APT[4]=gCANOPENConfig_APT.error_register_APT;
                CANOpenHW_SendMessage_APT(&gTxSDO_APT.BUF_APT[0], txID_APT, txDLC_APT, txData_APT);
                return 1;
            }
        ...
        Send_SDO_Abort_APT(SDO_ABORT_NOT_EXISTS,txID_APT, txDLC_APT, txData_APT);
        return 0;
    }
    if (cmd!=0x80)
...
void SDO_Write_APT(WORD Node_ID_APT, WORD index, BYTE subindex, uint32_T data,BYTE len)
{
	signed char k;
    gTxSDOw_APT.ID=0x600+Node_ID_APT;
 	gTxSDOw_APT.LEN=8;
	gTxSDOw_APT.BUF_APT[0]=0x23 | ((4-len)<<2);
	gTxSDOw_APT.BUF_APT[1]=(BYTE) (index & 0xFF);
	gTxSDOw_APT.BUF_APT[2]=(BYTE) ((index>>8) & 0xFF);
	gTxSDOw_APT.BUF_APT[3]= subindex;
	for (k=0;k<4;k++)
	  gTxSDOw_APT.BUF_APT[4+k]=0;
	switch(len)
	{
		case 1:
            gTxSDOw_APT.BUF_APT[4]= (BYTE) (data);
		break;
		case 2:
		    gTxSDOw_APT.BUF_APT[4]= (BYTE) (data);
		    gTxSDOw_APT.BUF_APT[5]= (BYTE) (data>>8);
		break;
		case 3:
		    gTxSDOw_APT.BUF_APT[4]= (BYTE) (data);
		    gTxSDOw_APT.BUF_APT[5]= (BYTE) (data>>8);
	  	    gTxSDOw_APT.BUF_APT[6]= (BYTE) (data>>16);
		break;
		case 4:
		    gTxSDOw_APT.BUF_APT[4]= (BYTE) (data);
		    gTxSDOw_APT.BUF_APT[5]= (BYTE) (data>>8);
	  	    gTxSDOw_APT.BUF_APT[6]= (BYTE) (data>>16);
		    gTxSDOw_APT.BUF_APT[7]= (BYTE) (data>>24);
		break;
...
//Considers valid data when rxDLC_APT>0
//Ouput data to be transmitted is considered valid when *txData_APT>0
void CANOpenSlave_ProcessStackAsyncRx_APT(uint32_T *txID_APT, uint8_T *txDLC_APT, uint8_T *txData_APT)
{
    BYTE    i;
//     gTimCnt_APT++; //always advancing time
       ...
       aux_gTimCnt_APT++; //always advancing time
   gTimCnt_APT=gTimCnt_APT;
    if (aux_gTimCnt_APT >= TimFactor_APT)
    {    
        gTimCnt_APT++;
        aux_gTimCnt_APT = 0;
    }
	...
	*txDLC_APT = 0; //nothing to send
	...
	if(gRxCAN_APT) {
        ...
        {// NMT Master Message
            if((gRxCAN_APT->BUF_APT[1]==gCANOPENConfig_APT.Node_ID_APT)||(gRxCAN_APT->BUF_APT[1]==0))
            {// NMT message is for this node or for all nodes
                switch (gRxCAN_APT->BUF_APT[0])
                {
                    case 0x01://Start node
                        gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0x05;
                        #if NR_OF_TPDOS_APT>0
                        Prepare_TPDOs_APT();
                        #endif
                        break;
                    case 0x02://Stop node
                        gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0x04;
                        break;
                    case 0x80:
                        gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0x7F;
                        break;
...
/**********************************************************************************************/
       ...
       if (gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]!=0x04)
       {// Node is NOT stopped
            if (gRxCAN_APT->ID==gCANOPENConfig_APT.Node_ID_APT+0x600)
            {// SDO Request
                i=Handle_SDO_Request_APT(&gRxCAN_APT->BUF_APT[0],txID_APT,txDLC_APT, txData_APT);//Returned value not used in this version
            }
        }
/***********************************************************************************************/
        ...
        if (gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]==0x05)
        {// Node is in operational
            ...
            while(i<NR_OF_RPDOS_APT)
            {
                ...
                {// RPDO match
				    memcpy(gRPDOConfig_APT[i].pData, &(gRxCAN_APT->BUF_APT[0]),gRPDOConfig_APT[i].LEN);
                    i=NR_OF_RPDOS_APT;
                }
                i++;
            }// for all RPDOs
    ...
    {
        //init heartbeat time
        gCANOPENConfig_APT.heartbeat_timestamp_APT=CANOpenHW_GetTime_APT()+gCANOPENConfig_APT.heartbeat_time_APT;
        //Send Boot-up message
        CANOpenHW_SendMessage_APT(&gCANOPENConfig_APT.heartbeat_msg_APT, txID_APT, txDLC_APT, txData_APT);
        #ifdef AUTOSTART
            gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0x05; //going into operational state
            #if NR_OF_TPDOS_APT>0
                Prepare_TPDOs_APT();
            #endif
        #else// AUTOSTART
            gCANOPENConfig_APT.heartbeat_msg_APT.BUF_APT[0]=0x7F; //going into pre-operational state
        #endif// AUTOSTART
    ...
    {// Node is in operational
        ...
        if(gTPDONr_APT>=NR_OF_TPDOS_APT)
        {
           ...
           #ifdef USE_EVENT_TIME_APT
            if((gTPDOConfig_APT[gTPDONr_APT].event_time_APT!=0)&& (CANOpenHW_IsTimeExpired_APT(gTPDOConfig_APT[gTPDONr_APT].event_timestamp_APT)))
            {//TPDO 'i' uses event timer and event timer is expired
             //Get application data
                memcpy(gTPDOConfig_APT[gTPDONr_APT].CAN.BUF_APT,gTPDOConfig_APT[gTPDONr_APT].pData, gTPDOConfig_APT[gTPDONr_APT].CAN.LEN);
                TransmitPDO_APT(gTPDONr_APT, txID_APT, txDLC_APT, txData_APT);
                return;
            }
            #endif //USE_EVENT_TIME
            #ifdef USE_INHIBIT_TIME_APT
            if(gTPDOConfig_APT[gTPDONr_APT].inhibit_time_APT!=0)
            {// Not using event timer, COS transmission
                if (gTPDOConfig_APT[gTPDONr_APT].inhibit_status_APT==2)
                {// New transmit message already waiting
                    if (CANOpenHW_IsTimeExpired_APT(gTPDOConfig_APT[gTPDONr_APT].inhibit_timestamp_APT))
                ...
                {// Inhibit status is 0 or 1
                 // Check if application data changed
                    if((memcmp(&gTPDOConfig_APT[gTPDONr_APT].CAN.BUF_APT[0],gTPDOConfig_APT[gTPDONr_APT].pData, gTPDOConfig_APT[gTPDONr_APT].CAN.LEN)!=0))
                    {// Application data changed!
                     //Copy application data
                         memcpy(&gTPDOConfig_APT[gTPDONr_APT].CAN.BUF_APT[0],gTPDOConfig_APT[gTPDONr_APT].pData, gTPDOConfig_APT[gTPDONr_APT].CAN.LEN);
                         if (gTPDOConfig[gTPDONr].inhibit_status_APT==0)
                         {// Inhibit time already expired
                             TransmitPDO_APT(gTPDONr_APT, txID_APT, txDLC_APT, txData_APT);
                             return;
                         }
                        //inhibit_status_APT is 1
                        gTPDOConfig_APT[gTPDONr_APT].inhibit_status_APT=2;//Wait for inhibit time to expire
                    }
                }
            }// Inhibit Time !=0
    ...
    //Produce Heartbeat
    if (gCANOPENConfig_APT.heartbeat_time_APT!=0)
    {
        if (CANOpenHW_IsTimeExpired_APT(gCANOPENConfig_APT.heartbeat_timestamp_APT))
        {
            CANOpenHW_SendMessage_APT(&gCANOPENConfig_APT.heartbeat_msg_APT, txID_APT, txDLC_APT, txData_APT);
            gCANOPENConfig_APT.heartbeat_timestamp_APT=CANOpenHW_GetTime_APT()+gCANOPENConfig_APT.heartbeat_time_APT;
        }
...
void CANOpenSlave_ProcessStack_APT(uint32_T rxID_APT, uint8_T rxDLC_APT, uint8_T const *rxData_APT, uint32_T *txID_APT, uint8_T *txDLC_APT, uint8_T *txData_APT)
{
	if(rxDLC_APT>0) {
		#ifdef INTERNAL_RX_QUEUE_APT
		...
		if(n != can_rx_queue_APT.out) {
			...
			memcpy(d->BUF_APT, rxData_APT, rxDLC_APT);
			d->ID = rxID_APT;
			d->LEN = rxDLC_APT;
		...
		#else
			gRxCAN_APT = &can_rx;
			memcpy(gRxCAN_APT->BUF_APT, rxData_APT, rxDLC_APT);
			gRxCAN_APT->ID = rxID_APT;
			gRxCAN_APT->LEN = rxDLC_APT;
		#endif
	}
	CANOpenSlave_ProcessStackAsyncRx_APT(txID_APT, txDLC_APT, txData_APT);
}

void CANOpenSlave_GetRPDO_APT(uint8_T no_APT, uint8_T *data_APT) {
	memcpy(data_APT, gRPDOConfig_APT[no_APT].pData, gRPDOConfig_APT[no_APT].LEN);
}
void CANOpenSlave_SetTPDO_APT(uint8_T no_APT, uint8_T const *data_APT) {
	memcpy(gTPDOConfig_APT[no_APT].pData, data_APT, 8);//gTPDOConfig_APT[no].CAN.LEN);
}
