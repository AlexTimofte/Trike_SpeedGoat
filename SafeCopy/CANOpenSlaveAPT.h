...
typedef struct
{
    BYTE        Node_ID_APT;                // Current Node ID (1-126)
    WORD        error_code_APT;             // Error code for OD entry [1003,00]
    WORD        Baudrate_APT;               // Current Baudrate_APT in kbit
    WORD        heartbeat_time_APT;         // Heartbeat time in ms
    WORD        heartbeat_timestamp_APT;    // timestamp_APT of last heartbeat
    CAN_MSG_APT heartbeat_msg_APT;          // Heartbeat message contents
    BYTE        error_register_APT;         // Error register for OD entry [1001,00]
...
typedef struct
{
    ...
    WORD    event_time_APT;             // Event timer in ms
    WORD    event_timestamp_APT;        // If event timer is used, this is the
                                    // timestamp_APT for the next transmission
#endif
#ifdef  USE_inhibit_time_APT
    WORD        inhibit_time_APT;       // Inhibit timer in ms
    WORD        inhibit_timestamp_APT;  // If inhibit timer is used, this is the
                                    // timestamp_APT for the next transmission
    BYTE        inhibit_status_APT;     // 0: Inhibit timer not started or expired
