...
//************************************************************************
//BYTE CANOpenHW_ReceiveMessage(CAN_MSG *pReceiveBuf)
//{
 //   if (gReceived)
 //   {
 //       gReceived=FALSE;
...
{
    CANOpenSlave_Init(500, OD_NODEID, OD_HEARTBEAT);
    #if NR_OF_TPDOS>0
        // TPDO1, default ID (0x180+NodeID),100ms event, 0ms inhibite, 4 bytes
        // Transmit trigger: 100 ms event time
        CANOpenSlave_InitTPDO(1,   0x0,  10, 0, 8,  &TPDO01.Message);
       // CANOpenSlave_InitTPDO(2, 0x285, 500, 0, 8, &TPDO02.Message);
    #endif

    //09.10.2010 Receptie SDO Controllere motoare prin RTPDs MPC555 - slave
    #if NR_OF_RPDOS>0
